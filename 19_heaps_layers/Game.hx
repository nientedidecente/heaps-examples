 ...
 * https://heaps.io/api/h2d/Layers.html
 * h2d.Layers class could be used as a lighter alternative to object hierarchies.
 * As you could guess from its name, this class organizes its children into separate layers.
 * Layers are divided by their indexes. Layers will be drawn according to their indexes, lowest first.
 * Internally Heaps uses Layers in h2d.CdbLevel, where each "layer" is used for separate tiles from tilemap.
 * https://heaps.io/api/h2d/CdbLevel.html
 *
 * In this example we'll try to load Tiled map.
 * This example is the modified version of "Drawing tiles" example from official Heaps site:
 * https://heaps.io/documentation/drawing-tiles.html
 *
 * Assets for this project are taken from the official Tiled examples:
